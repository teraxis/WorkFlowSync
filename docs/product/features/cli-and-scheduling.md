...
| Команда | Дія |
|---------|-----|
| `sync --once` | один прохід усіх пар і вихід. Режим для Планувальника завдань |
...
Іменований `Mutex` `Local\WorkFlowSync.<hash шляху конфігу>`: другий запуск на тому ж конфігу
завершується з кодом 1 і повідомленням. Це захищає від накладання проходів, коли Планувальник
